    ...
    ) : ILoginService
{
    ...
    public async Task<Result<TokenData>> LoginAsync(LoginRequest request, CancellationToken cancellationToken)
    {
        return await _userDbRepository.GetUserByUsernameAsync(request.UserName, cancellationToken)
            ...
            .Map(tokens => new TokenData(tokens.AccessToken, tokens.RefreshToken, "Bearer", tokens.ExpiresInMinutes));
    }
